...
export default async function PaathListPage() {
  ...
  const activePaaths = paaths.filter((p) => p.active);

  return (
    <div className="page-section">
      <div className="container">
        <div className="list-page-header">
          <h1 className="list-page-title">ਪਾਠ</h1>
          <p className="list-page-sub">ਗੁਰਬਾਣੀ ਦੇ ਵੱਖ-ਵੱਖ ਪਾਠ</p>
        </div>

        <div className="list-page-grid">
          {activePaaths.length === 0 ? (
            <div className="empty-state" style={{ gridColumn: '1 / -1' }}>
              <p>ਕੋਈ ਪਾਠ ਉਪਲਬਧ ਨਹੀਂ ਹਨ।</p>
            </div>
          ) : activePaaths.map((p) => (
            <Link key={p._id} href={`/paath/${p.slug}`} className="list-page-card">
              <div className="list-page-card-thumb">
                {p.thumbnail ? (
                  <img src={getThumbnailUrl(p.thumbnail)} alt={p.title} />
                ) : (
                  <span>☬</span>
                )}
              </div>
              ...
              <div className="list-page-card-footer">
                <span className="count-badge">{p.entryCount} {p.entryCount === 1 ? 'entry' : 'entries'}</span>
      ...
      <style>{`
        ...
        .list-page-card-thumb { height: 160px; background: linear-gradient(135deg, #1e1a10, #2d2510); display: flex; align-items: center; justify-content: center; overflow: hidden; }
