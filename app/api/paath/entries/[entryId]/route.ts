import { NextRequest, NextResponse } from 'next/server';
import { removeEntry } from '@/services/paathService';
import { requireAdmin } from '@/lib/apiAuth';
import { ADMIN_MUTATION_LIMIT, enforceRateLimit } from '@/lib/rateLimit';

interface Params {
  params: Promise<{ entryId: string }>;
}

export async function DELETE(req: NextRequest, { params }: Params) {
  try {
    const auth = await requireAdmin();
    if (!auth.authorized) return auth.response;
    const limited = enforceRateLimit(req, ADMIN_MUTATION_LIMIT);
    if (limited) return limited;

    const { entryId } = await params;
    const result = await removeEntry(entryId);
    return NextResponse.json({ success: true, message: 'Entry removed', archived: result.archived });
  } catch (error) {
    console.error('DELETE /api/paath/entries/[entryId]', error);
    return NextResponse.json({ success: false, error: 'Failed to remove entry' }, { status: 500 });
  }
}
